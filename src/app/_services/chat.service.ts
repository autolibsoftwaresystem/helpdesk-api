...
export class ChatService {
  ...
  getMessages(agent_employee_id,active_agent_employee_id , lastid = 0) {

    let chat_id = '';
    let chat_id2 = '';

      chat_id = agent_employee_id+'||'+ active_agent_employee_id;
      chat_id2 = active_agent_employee_id+'||'+ agent_employee_id;
  ...
  getUserMessages(user_email_id) {
    ...
    let request = {
      ChannelName: "WEB", chat_id: chat_id,chat_id2:chat_id2
    }
    return this.http.post(this.util.getBaseURL() + 'chat/getUserMessages', request);
  }

  getInfoDetails(id) {
    let request = { "id": id };
    return this.http.post(this.util.getBaseURL() + '/agent/get-info-details', request);
  }
