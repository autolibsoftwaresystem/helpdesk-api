...
export class ChatComponent implements OnInit  , OnDestroy {
  ...
  constructor(private util : UtilService,private cs : ChatService , private tst : ToastrService,public auth: AuthService,private is: InstituteService){
  ...
  ngOnInit() {    
    ...
    console.log(this.instContactDetails.emailId);
    this.user_email_id = this.instContactDetails.emailId;
    this.changeActiveChat(this.user_email_id);
    this.instituteDetails = this.auth.getInstituteDetails();
    this.instituteId = this.instituteDetails.instituteId;
  ...
  webSocketEndPoint: string = this.util.getBaseURL()+ 'ws';
    ...
    _send(str) {
          ...
          let _message_to_send = {
            chatId : this.getChatID(this.user_email_id),
            agentMailId : '',
            userMailId : this.user_email_id,
            adminMemberCode : this.getAdminMemberCode(this.active_chat.agent_email_id),
            message : str,
            messageBy :  this.user_email_id,
            messageByName : this.user_email_id,
          ...
          this.stompClient.send("/app/hello/agent", {}, JSON.stringify(_message_to_send));
      }
    ...
    getChatID(user_id):String{
        
      let chat_id = '';
        chat_id = user_id+'||'+ 'agent';
        return chat_id;
    }
    ...
    changeActiveChat(user_email_id){
      ...
      this.cs.getUserMessages(user_email_id).subscribe(res=>{
        console.log(res);
